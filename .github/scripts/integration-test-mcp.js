...
async function runTest(liveIssues) {
  ...
  const issuesWithSubIssues = liveIssues.filter(i => 
    i.trackedIssues && i.trackedIssues.totalCount > 0
  );
  ...
  if (issue79) {
    ...
    console.log(`  Assignees: ${issue79.assignees.nodes.map(a => a.login).join(', ') || 'None'}`);
    console.log(`  trackedIssues.totalCount (from GitHub): ${issue79.trackedIssues.totalCount}`);
    ...
    if (result79.shouldSkip && result79.reason === "has sub-issues") {
      console.log(`  ✅ PASS: Issue #79 is correctly SKIPPED because it has sub-issues`);
    } else {
  ...
  if (issuesWithSubIssues.length > 0) {
    issuesWithSubIssues.forEach(issue => {
      ...
      console.log(`    Sub-issues count (from GitHub): ${issue.trackedIssues.totalCount}`);
      console.log(`    Should skip: ${result.shouldSkip} (${result.reason})`);
      console.log();
    });
    console.log(`  ✅ PASS: Found and correctly identified ${issuesWithSubIssues.length} parent issue(s) with sub-issues`);
  } else {
  ...
  if (assignable) {
    ...
    console.log(`  ✅ PASS: Successfully found assignable issue from live data`);
  } else {
    console.log(`  No assignable issues found`);
    console.log(`  ℹ️  All ${liveIssues.length} open issues are either assigned or have sub-issues`);
  }
  console.log();
  ...
  if (assignableIssues.length > 0) {
    ...
    if (assignableIssues.length > 5) {
      console.log(`  ... and ${assignableIssues.length - 5} more assignable issue(s)`);
      console.log();
    }
    console.log(`  ✅ PASS: Found ${assignableIssues.length} assignable issue(s) from live data`);
  } else {
  ...
  if (issuesWithSubIssues.length > 0) {
    ...
    console.log(`  Has sub-issues (from GitHub): ${firstParent.trackedIssues.totalCount}`);
    console.log(`  Should skip (allowParentIssues=false): ${helpers.shouldSkipIssue(parsedParent, false).shouldSkip}`);
    console.log(`  Should skip (allowParentIssues=true): ${resultWithOverride.shouldSkip}`);
    console.log(`  Reason: ${resultWithOverride.reason || 'None - parent issues allowed'}`);
    
    if (!resultWithOverride.shouldSkip) {
      console.log(`  ✅ PASS: Parent issue becomes assignable with allowParentIssues=true`);
    } else {
      console.log(`  ❌ FAIL: Parent issue should be assignable when allowParentIssues=true`);
    }
    console.log();
  }
...
if (require.main === module) {
  ...
  console.log("Use the task tool with description 'Run integration test with live GitHub data'");
}
