...
async function runIntegrationTest() {
  ...
  try {
    ...
    if (issue79) {
      ...
      console.log(`  Has sub-issues (from GitHub): ${issue79.trackedIssues.totalCount}`);
    ...
    if (issuesWithSubIssues.length > 0) {
      ...
      console.log(`  ✅ PASS: Found ${issuesWithSubIssues.length} issue(s) with sub-issues`);
    } else {
    ...
    if (assignableIssues.length > 0) {
      ...
      console.log(`  ✅ PASS: Found ${assignableIssues.length} assignable issue(s)`);
    } else {
    ...
    if (issuesWithSubIssues.length > 0) {
      ...
      if (!resultWithOverride.shouldSkip) {
        console.log(`  ✅ PASS: Parent issue is assignable with allowParentIssues=true`);
      } else {
